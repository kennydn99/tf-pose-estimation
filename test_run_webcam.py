...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            #Calculate the angle for right side
            pc = perpCoord(neck, RS)
            angle = calculate_angle(pc, RS, RE)
            formatted_angle = "{:.2f}".format(angle)

            #calculate angle for left side
            left_pc = perpCoord(neck, LS)
            left_angle = calculate_angle(left_pc, LS, LE)
            left_formatted_angle = "{:.2f}".format(left_angle)

            #visualizing angles on image
            img_h, img_w = image.shape[:2]
            cv2.putText(
                        image, formatted_angle,
            ...
            cv2.circle(
                        image,
                        tuple(np.multiply(pc, [img_w, img_h]).astype(int)),
                        3,
                        (0,0,255),
                        thickness=3,
                        lineType=8,
            ...
            cv2.circle(
                        image,
                        tuple(np.multiply(left_pc, [img_w, img_h]).astype(int)),
                        3,
                        (0,0,255),
                        thickness=3,
                        lineType=8,
